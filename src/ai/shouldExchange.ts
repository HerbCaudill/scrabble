...
/** Score thresholds below which exchange is recommended, by difficulty. */
const EXCHANGE_THRESHOLDS: Record<Difficulty, number> = {
  easy: 5,
  medium: 8,
  hard: 12,
}
...
/**
 * Heuristic for whether the computer player should exchange tiles instead of playing.
 * Considers the best available score and rack letter quality.
 * Returns false if the rack is empty.
 */
export const shouldExchange = (
  ...
  // No moves available means we should exchange (or pass)
  if (moves.length === 0) return true

  const bestScore = moves[0].score
  const threshold = EXCHANGE_THRESHOLDS[diff]

  // If the best score is above threshold, no need to exchange
  ...
  // Exchange if rack is mostly bad letters and best score is low
  return badRatio >= 0.4
}
