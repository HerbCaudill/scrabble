...
describe("swapPlayerTiles", () => {
  ...
  it("advances to the next player", () => {
    const state = createGame(["Alice", "Bob"])
    const result = swapPlayerTiles(state, [0])
  ...
  it("adds a swap record to move history", () => {
    const state = createGame(["Alice", "Bob"])
    const result = swapPlayerTiles(state, [0, 1])
    expect(result.moveHistory).toHaveLength(1)
    expect(result.moveHistory[0].actionType).toBe("swap")
  ...
  it("throws when fewer than 7 tiles remain in the bag", () => {
    ...
    expect(() => swapPlayerTiles(state, [0])).toThrow()
  })

  it("does not mutate the original state", () => {
    const state = createGame(["Alice", "Bob"])
    const originalRack = [...state.players[0].rack]
    swapPlayerTiles(state, [0, 1])
